...
CREATE OR REPLACE FUNCTION update_user_updated_at_column() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at := now();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Create trigger for user
CREATE OR REPLACE FUNCTION update_news_updated_at_column() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at := now();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Create trigger for user
CREATE OR REPLACE FUNCTION update_booking_updated_at_column() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at := now();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Create trigger for user
CREATE OR REPLACE FUNCTION update_ticket_updated_at_column() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at := now();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Trigger to update updated_at column on Users table
CREATE TRIGGER set_updated_at BEFORE
UPDATE ON Users FOR EACH ROW EXECUTE FUNCTION update_user_updated_at_column();
-- Trigger to update updated_at column on Customers table
CREATE TRIGGER set_updated_at BEFORE
UPDATE ON News FOR EACH ROW EXECUTE FUNCTION update_news_updated_at_column();
-- Trigger to update updated_at column on Bookings table
CREATE TRIGGER set_updated_at BEFORE
UPDATE ON Bookings FOR EACH ROW EXECUTE FUNCTION update_booking_updated_at_column();
-- Trigger to update updated_at column on Tickets table
CREATE TRIGGER set_updated_at BEFORE
UPDATE ON Tickets FOR EACH ROW EXECUTE FUNCTION update_ticket_updated_at_column();
-- Đảm bảo ngày đi trước ngày đến
ALTER TABLE Flights
ADD CONSTRAINT departure_before_arrival CHECK (departure_time < arrival_time);
INSERT INTO Users (
    email,
    hashed_password,
    first_name,
...
VALUES (1, 'economy', 1000000, 'Active', 1, 1),
...
VALUES (1, 'John', 'Doe', '1234567890', 'Male'),
...
VALUES (
    ...
    'Full content about delays...',
    'https://picsum.photos/536/354',
    2
  ),
  (
    'New Routes Announced',
    'Vietnam Airlines announces new routes',
    'Full content about new routes...',
    'https://picsum.photos/536/354',
    2
  ),
  (
    'Special Discounts for Summer',
    'Enjoy up to 50% off on selected flights',
    'Full content about discounts...',
    'https://picsum.photos/536/354',
    2
  ),
  (
    'New Aircraft Added to Fleet',
    'Vietnam Airlines adds new aircraft to its fleet',
    'Full content about new aircraft...',
    'https://picsum.photos/536/354',
    2
  ),
  (
    'Safety Measures Enhanced',
    'Enhanced safety measures for passengers',
    'Full content about safety measures...',
    'https://picsum.photos/536/354',
    3
  ),
  (
    'Loyalty Program Updates',
    'Exciting updates to our loyalty program',
    'Full content about loyalty program...',
    'https://picsum.photos/536/354',
    3
  ),
  (
    'Holiday Travel Tips',
    'Tips for a smooth holiday travel experience',
    'Full content about travel tips...',
    'https://picsum.photos/536/354',
    4
  ),
  (
    'New International Destinations',
    'Explore new international destinations with us',
    'Full content about destinations...',
    'https://picsum.photos/536/354',
    4
  );